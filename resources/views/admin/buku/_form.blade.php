<div class="row g-4">
    ...
    <div class="col-12">
        ...
        <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama" name="nama"
            value="{{ old('nama', $buku?->nama ?? '') }}" placeholder="Contoh: Belajar Laravel 11 untuk Pemula" required />
        @error('nama')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="col-md-6">
        ...
        <input type="text" class="form-control @error('pengarang') is-invalid @enderror" id="pengarang"
            ...
            placeholder="Contoh: Budi Raharjo" required />
        @error('pengarang')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="col-12">
        <div class="form-check form-switch">
            <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1"
                @checked(old('is_active', $buku?->is_active ?? true))>
            <label class="form-check-label" for="is_active">Tampilkan buku ini di katalog</label>
        </div>
    </div>
</div>
